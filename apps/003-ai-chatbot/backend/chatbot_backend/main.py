...
from chatbot_backend.adapters.api import auth_routes, chat_routes, task_routes
from chatbot_backend.adapters.db.session import init_db
...
if __name__ == "__main__":
    ...
    uvicorn.run("chatbot_backend.main:app", host="0.0.0.0", port=port, reload=False)


app.include_router(auth_routes.router, prefix="/api/v1/auth", tags=["auth"])
app.include_router(task_routes.router, prefix="/api/v1/tasks", tags=["tasks"])
app.include_router(chat_routes.router)  # Chat routes are prefixed in the router
