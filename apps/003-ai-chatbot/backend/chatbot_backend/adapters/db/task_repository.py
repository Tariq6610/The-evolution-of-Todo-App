...
class TaskTable(SQLModel, table=True):
    ...
    status: str
    priority: str
    tags: str  # Stored as comma-separated values for simplicity
    ...
    def get_all(
        ...
        if search:
            statement = statement.where(
                (TaskTable.title.contains(search))  # type: ignore[attr-defined]
                | (
                    cast(Any, TaskTable.description).is_not(None)
                    & cast(Any, TaskTable.description).contains(search)
                )
            )
        ...
        if tag:
            statement = statement.where(cast(Any, TaskTable.tags).contains(tag))

        # Apply sorting
        if sort_by:
            if sort_by == "created_at":
                statement = statement.order_by(TaskTable.created_at)  # type: ignore[arg-type]
            elif sort_by == "updated_at":
                statement = statement.order_by(TaskTable.updated_at)  # type: ignore[arg-type]
            elif sort_by == "title":
