      ...
      >
        <div className="flex items-start gap-3 sm:gap-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            onClick={onToggle}
            className={`mt-0.5 flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 flex items-center justify-center transition-all ${task.status === "COMPLETED" ? "bg-green-500 border-green-500 dark:border-green-500" : "border-gray-300 dark:border-gray-600 hover:border-green-500"}`}
          >
          ...
          <div className="flex-1 min-w-0">
            <motion.p
              ...
              className={`text-sm sm:text-base font-medium line-clamp-1 ${task.status === "COMPLETED" ? "line-through text-gray-400 dark:text-gray-500" : "text-gray-900 dark:text-white"}`}
            >
              {task.title}
            </motion.p>
            ...
            >
              ...
              <motion.span
                ...
                className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${task.status === "COMPLETED" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"}`}
              >
                {task.status === "COMPLETED" ? (
