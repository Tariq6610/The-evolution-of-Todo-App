...
from sqlmodel import Field, Session, SQLModel, col, select, or_, and_
...
class TaskTable(SQLModel, table=True):
    ...
    def to_domain(self) -> DomainTask:
        ...
        # but return empty list if empty or just whitespace
        if self.tags:
...
class SQLModelTaskRepository(StoragePort):
    ...
    def get(self, task_id: str) -> DomainTask | None:
    ...
    def get_all(
        ...
        if search:
            search_condition = or_(
                col(TaskTable.title).contains(search),
                and_(
                    col(TaskTable.description).is_not(None),
                    col(TaskTable.description).contains(search),
                ),
            )
        ...
        if status is not None and status != "None":
            try:
                valid_status = TaskStatus(status.lower())
                statement = statement.where(TaskTable.status == valid_status.value)
            except ValueError:
                pass

        if priority is not None and priority != "None":
            try:
                valid_priority = Priority(priority.lower())
                statement = statement.where(TaskTable.priority == valid_priority.value)
            except ValueError:
                pass

        if tag:
            statement = statement.where(TaskTable.tags.contains(tag))  # type: ignore

        # Apply sorting
        if sort_by:
            if sort_by == "created_at":
                statement = statement.order_by(TaskTable.created_at)  # type: ignore
            elif sort_by == "updated_at":
                statement = statement.order_by(TaskTable.updated_at)  # type: ignore
            elif sort_by == "title":
                ...
                priority_order = case(
                    (TaskTable.priority == "HIGH", 1),  # type: ignore
                    (TaskTable.priority == "MEDIUM", 2),  # type: ignore
                    (TaskTable.priority == "LOW", 3),  # type: ignore
                )
                ...
                priority_order = case(
                    (TaskTable.priority == "LOW", 1),  # type: ignore
                    (TaskTable.priority == "MEDIUM", 2),  # type: ignore
                    (TaskTable.priority == "HIGH", 3),  # type: ignore
                )
                statement = statement.order_by(priority_order)
